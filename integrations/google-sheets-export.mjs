...
if (!run) throw new Error('No crawler run found');
const results = resultsDb.prepare('SELECT * FROM crawl_results WHERE source_run_id=? ORDER BY id').all(run.id);
const response = await fetch(endpoint, { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({ token, run:{...run,source_run_id:run.id,runId:run.id}, results }) });
