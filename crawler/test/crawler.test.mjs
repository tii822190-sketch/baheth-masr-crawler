...
import { buildSearchKeywords, createProgressReporter, diagnoseReview, errorCode, isComplete, shouldUseBrowserFallback } from '../src/crawl.mjs';
...
test('prefers declared favicon and normalizes ./favicon.ico to the site root', () => { const x=extractHtml('<link rel="icon" href="./favicon.ico"><meta property="og:image" content="https://cdn.example.com/social-card.png">','https://quran.com/ar/page'); assert.equal(x.iconUrl,'https://quran.com/favicon.ico'); });
test('falls back to the site-root favicon.ico before Open Graph image metadata', () => { const x=extractHtml('<meta property="og:image" content="https://cdn.example.com/social-card.png">','https://quran.com/ar/page'); assert.equal(x.iconUrl,'https://quran.com/favicon.ico'); });
test('reports batch progress every 20% and flushes page details once per batch', () => { const progressLines=[]; const detailWrites=[]; const reporter=createProgressReporter(10,{writeProgress:(line)=>progressLines.push(line),writeDetails:(text)=>detailWrites.push(text)}); for(let i=1;i<=10;i++) reporter.record({url:`https://example.com/${i}`,outcome:'indexed'}); reporter.flush(); assert.deepEqual(progressLines.map((line)=>line.match(/\d+%/)[0]),['20%','40%','60%','80%','100%']); assert.equal(detailWrites.length,1); assert.equal(detailWrites[0].trim().split('\n').length,10); assert.match(detailWrites[0],/crawler_page/); });
