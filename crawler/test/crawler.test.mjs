...
test('light extraction keeps only index fields with a 250-character description limit', () => { const x=extractLightHtml('<title>عنوان</title><meta name="description" content="وصف مختصر"><link rel="icon" href="/favicon.ico"><main><p>'+('محتوى مفيد للبحث '.repeat(80))+'</p><a href="/secret">رابط</a></main>','https://example.com/page'); assert.equal(x.iconUrl,'https://example.com/favicon.ico'); assert.equal(x.summary,'وصف مختصر'); assert.equal(x.searchSnippet,x.summary); assert.doesNotMatch(x.searchSnippet,/عنوان —/); assert.equal(x.links.length,0); assert.equal(x.internalLinks.length,0); assert.ok(x.description.length<=250); assert.ok(x.searchSnippet.length<=320); assert.ok(x.extractedText.length<=900); assert.ok(x.keywords.length<=3); assert.ok(x.categories.length<=3); });
test('light extraction falls back from empty HTML descriptions', () => { const x=extractLightHtml('<title>مكتبة القرآن الصوتية</title><meta name="description" content="&lt;p&gt;&lt;br&gt;&lt;/p&gt;"><main><p>تصفح واستماع وتحميل سور القرآن الكريم.</p></main>','https://mp3quran.net/'); assert.match(x.summary,/تصفح واستماع وتحميل سور القرآن الكريم/); assert.equal(x.searchSnippet,x.summary); assert.doesNotMatch(x.description,/<[a-z]/i); assert.match(x.description,/مكتبة القرآن/); });
