...
test('removes CSS and JavaScript and keeps useful structured content', () => {
  ...
  assert.match(x.extractedText, /عنوان مفيد/);
  assert.match(x.extractedText, /هذا نص مفيد/);
  assert.doesNotMatch(x.extractedText, /color:red|window\.secret|حقوق النشر|Menu/);
});

test('cleans quoted embedded social URLs', () => {
  const x = extractHtml('<a href="&quot;https://facebook.com/page">Facebook</a>', 'https://example.com/');
  assert.equal(x.links[0], 'https://facebook.com/page');
});
