...
test('light extraction keeps only index fields and bounded summary data', () => { const x=extractLightHtml('<title>عنوان</title><meta name="description" content="وصف مختصر"><link rel="icon" href="/favicon.ico"><main><p>'+('محتوى مفيد للبحث '.repeat(80))+'</p><a href="/secret">رابط</a></main>','https://example.com/page'); assert.equal(x.iconUrl,'https://example.com/favicon.ico'); assert.equal(x.links.length,0); assert.equal(x.internalLinks.length,0); assert.ok(x.summary.length<=600); assert.ok(x.searchSnippet.length<=320); assert.ok(x.extractedText.length<=900); assert.ok(x.keywords.length<=3); assert.ok(x.categories.length<=3); });
test('light extraction falls back from empty HTML descriptions', () => { const x=extractLightHtml('<title>مكتبة القرآن الصوتية</title><meta name="description" content="&lt;p&gt;&lt;br&gt;&lt;/p&gt;"><main><p>تصفح واستماع وتحميل سور القرآن الكريم.</p></main>','https://mp3quran.net/'); assert.doesNotMatch(x.summary,/<[a-z]/i); assert.match(x.summary,/مكتبة القرآن/); });
