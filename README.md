...
يشغّل `tools/discovery-spider.mjs` اكتشاف موقع واحد في كل مرة. الموقع المختار هو أول موقع بحالة `pending` أو `not_pages`، أو موقع محدد عبر `DISCOVERY_SITE_URL`. تعاد محاولة المواقع `not_pages` بمسارات Sitemap المحدّثة في التشغيلات اللاحقة. لا تُستأنف المواقع بحالة `incomplete` افتراضيًا؛ يمكن تفعيل ذلك عبر `DISCOVERY_RESUME_INCOMPLETE=true`.

يقرأ الاكتشاف قواعد `robots.txt` ويجمع جميع توجيهات `Sitemap` والخرائط المعلنة في HTML، ثم يجرّب مسارات شائعة إضافية تشمل خرائط WordPress والأخبار والمقالات والصفحات وHTML وJSON وURL lists والصيغ المضغوطة. يتابع خرائط الفهارس والروابط المتداخلة ضمن حدود التزامن، ويقبل XML القياسي ذا البادئات الاسمية إضافةً إلى RSS وAtom وRDF وJSON Feed. تسجل نتائج كل ملف: رمز HTTP، ونوع المحتوى، وصيغته، وعدد الصفحات والخرائط الفرعية، وسبب الفشل أو الرفض. يفحص الاكتشاف الروابط مقابل قواعد `Allow` و`Disallow` قبل إدخالها إلى الطابور. إذا تعذر الوصول إلى `robots.txt` بسبب خطأ شبكة أو خادم، يؤجل اكتشاف الموقع بدل تجاوزه. أما غياب الملف بردّ 4xx فيُعامل على أنه لا توجد قواعد منشورة.

يطبّق الاكتشاف التطبيع على الروابط، مثل إزالة اسم المضيف `www` والهاش ومنافذ HTTP/HTTPS الافتراضية ومعاملات التتبع الشائعة فقط. يحافظ على معاملات المحتوى، ويرفض روابط المنافذ المختلفة عن الموقع، ولا يستبعد مسارات عامة مثل `category` و`pages` تلقائيًا. يبقى استبعاد الملفات غير المناسبة والمسارات الإدارية وصفحات البحث والتغذية المحددة. يفحص الاكتشاف صفحة البداية حتى إن لم تعمل الخرائط، ويصعّد الصفحات الشحيحة أو هياكل تطبيقات JavaScript إلى Chromium، ثم يتتبع الروابط الداخلية مرتبة بالأولوية حتى عمق وحدّ زمني وعدد صفحات مضبوط. يضمن القيد الفريد في `site_pages.url` تجاهل الرابط إذا وصل من أكثر من Sitemap أو خلاصة أو صفحة. الحد الافتراضي هو 5,000 صفحة مقبولة و2,000 Sitemap لكل موقع، و90 ثانية لمسار الاستكشاف عبر المتصفح.
...
تُعدّ صلاحية الصفحات للمحتوى المؤرشف مسؤولية الزاحف، ولذلك يعيد الاكتشاف `validation: deferred_to_crawler`. قواعد `robots.txt` هي توجيهات استبعاد للزواحف وليست آلية مصادقة أو حماية لمحتوى خاص.
...
تنتقل الصفحة غير المكتملة أولًا إلى `needs_review` ثم تُتاح لإعادة المحاولة المحددة. يبث log كل صفحة عند اكتمالها مباشرةً، ويحفظ الـartifact أحداث الصفحات في `primary-crawler-result.jsonl` مع ملخص الدفعة في `primary-crawler-result.json`. تشمل التفاصيل الصفحات الناجحة والفاشلة، وسبب الفشل والحقول الناقصة وحالة HTTP ونوع المحتوى وطريقة الجلب والمدة وعدد المحاولات ونصًا مقتطفًا ومحاولة المتصفح ونتيجة مقارنة HTTP/Chromium. لا تُضاف أعمدة تشخيص إلى قاعدة SQLite؛ وعند استنفاد المحاولات تنتقل الصفحة إلى `corrupt` وتُحذف من الطابور، مع بقاء تفاصيلها في سجل/artifact التشغيل.
