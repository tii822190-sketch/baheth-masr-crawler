...
تنتقل الصفحة غير المكتملة أولًا إلى `needs_review` ثم تُتاح لإعادة المحاولة المحددة. يسجل log التشغيل وملف الـartifact تفاصيل كل صفحة—الناجحة والفاشلة—بما في ذلك السبب والحقول الناقصة وحالة HTTP ونوع المحتوى وطريقة الجلب والمدة وعدد المحاولات ومحاولة المتصفح. تُجمع أسباب الإخفاق أيضًا في ملخص JSON لكل دفعة. لا تُضاف أعمدة تشخيص إلى قاعدة SQLite؛ وعند استنفاد المحاولات تنتقل الصفحة إلى `corrupt` وتُحذف من الطابور، مع بقاء تفاصيلها في log/artifact التشغيل.
