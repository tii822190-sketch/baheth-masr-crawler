...
requests.push({ type: 'execute', stmt: { sql: `INSERT INTO site_search_fts (record_type,record_id,site_id,priority,title,description,content,keywords,categories,search_text) SELECT 'site',printf('%d',id),id,priority,name,description,'',keywords,categories,search_text FROM sites WHERE canonical_url=?`, args: [arg('text', rootUrl)] } });
