...
const requests = [{ type: 'execute', stmt: { sql: `CREATE VIRTUAL TABLE IF NOT EXISTS site_search_fts_v2 USING fts5(record_type UNINDEXED,record_id UNINDEXED,site_id UNINDEXED,priority UNINDEXED,title,description,content,keywords,categories,search_text, tokenize='unicode61')` } }, { type: 'execute', stmt: { sql: `INSERT OR IGNORE INTO sites (name,url,canonical_url,description,icon_url,keywords,categories,priority,search_text,status,last_verified_at,failed_attempts,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,'active',CURRENT_TIMESTAMP,0,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`, args: [arg('text', rootName), arg('text', rootUrl), arg('text', rootUrl), arg('text', rootDescription), arg('text', rootRow.icon_url || ''), arg('text', rootKeywords), arg('text', rootCategory), arg('integer', priority), arg('text', rootRow.search_text || '')] } }];
for (const row of rows) {
  ...
  requests.push({ type: 'execute', stmt: { sql: `DELETE FROM site_search_fts_v2 WHERE record_type='page' AND record_id IN (SELECT CAST(id AS TEXT) FROM site_pages WHERE canonical_url=? OR url=?)`, args: [arg('text', canonical), arg('text', row.requested_url || canonical)] } });
  requests.push({ type: 'execute', stmt: { sql: `INSERT INTO site_search_fts_v2 (record_type,record_id,site_id,priority,title,description,content,keywords,categories,search_text) SELECT 'page',CAST(id AS TEXT),site_id,priority,title,description,content,keywords,categories,search_text FROM site_pages WHERE canonical_url=?`, args: [arg('text', canonical)] } });
}
...
requests.push({ type: 'execute', stmt: { sql: `DELETE FROM site_search_fts_v2 WHERE record_type='site' AND site_id=(SELECT id FROM sites WHERE canonical_url=?)`, args: [arg('text', rootUrl)] } });
