...
for (const row of rows) {
  ...
  requests.push({ type: 'execute', stmt: { sql: `INSERT INTO site_search_fts (record_type,record_id,site_id,priority,title,description,content,keywords,categories,search_text) SELECT 'page',CAST(id AS TEXT),site_id,priority,title,description,content,keywords,categories,search_text FROM site_pages WHERE canonical_url=?`, args: [arg('text', canonical)] } });
}
...
requests.push({ type: 'execute', stmt: { sql: `DELETE FROM site_search_fts WHERE record_type='site' AND site_id=(SELECT id FROM sites WHERE canonical_url=?)`, args: [arg('text', rootUrl)] } });
requests.push({ type: 'execute', stmt: { sql: `INSERT INTO site_search_fts (record_type,record_id,site_id,priority,title,description,content,keywords,categories,search_text) SELECT 'site',CAST(id AS TEXT),id,priority,name,description,'',keywords,categories,search_text FROM sites WHERE canonical_url=?`, args: [arg('text', rootUrl)] } });
