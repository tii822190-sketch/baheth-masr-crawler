...
const response = await fetch(endpoint, { method: 'POST', headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' }, body: JSON.stringify({ requests: [{ type: 'execute', stmt: { sql: 'SELECT id,site_id,url,canonical_url,crawl_status FROM site_pages ORDER BY id' } }, { type: 'close' }] }) });
...
const urls = rows.map((r) => r.canonical_url || r.url).filter(Boolean);
...
const out = [
  ...
  '',
  ...rows.map((r, i) => `${i + 1}. ${r.canonical_url || r.url} [status=${r.crawl_status || 'pending'}]`),
  '',
