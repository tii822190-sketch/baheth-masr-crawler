...
const maxSites = Math.max(1, Number(process.env.MONITOR_MAX_SITES || 25));
const maxNewPages = Math.max(1, Number(process.env.MONITOR_MAX_NEW_PAGES || 1000));
const browserBudgetMs = Math.max(1000, Number(process.env.MONITOR_BROWSER_BUDGET_MS || 6000));
const maxMapsPerSite = Math.max(1, Number(process.env.MONITOR_MAX_MAPS_PER_SITE || 40));
...
async function sitemapPages(siteId, root, siteCap) {
  ...
  while (pending.length && seen.size < maxMapsPerSite && added < siteCap) { const map = pending.shift(); if (seen.has(map)) continue; seen.add(map); const body = await text(map); if (!body || /\/robots\.txt$/i.test(map)) continue; const locs = /<(?:urlset|sitemapindex)\b/i.test(body) ? [...body.matchAll(/<loc[^>]*>\s*([^<]+?)\s*<\/loc>/gi)].map((m) => m[1].trim()) : body.split(/\r?\n/).map((x) => x.trim()).filter(Boolean); const index = /<sitemapindex\b/i.test(body); for (const raw of locs) { let url; try { url = new URL(raw, map).toString(); } catch { continue; } if (!allowed(url, root)) continue; if (index || /sitemap(?:[-_].*)?\.xml(?:\.gz)?$/i.test(url)) pending.push(url); else if (add(siteId, root, url)) added += 1; if (added >= siteCap) break; } }
...
for (const site of sites) { if (added >= maxNewPages) break; const root = site.url.endsWith('/') ? site.url : `${site.url}/`; const siteCap = Math.min(maxNewPages - added, Math.max(1, Math.ceil(maxNewPages / maxSites))); try { const result = await sitemapPages(site.id, root, siteCap); let siteAdded = result.added; if (!siteAdded) { const home = (await text(root)) || await browserHtml(root); if (home) { const meta = extractHtml(home, root, 'text/html'); for (const link of meta.internalLinks) { if (add(site.id, root, link)) { siteAdded += 1; if (siteAdded >= siteCap) break; } } } } db.prepare('UPDATE sites SET last_checked_at=CURRENT_TIMESTAMP WHERE id=?').run(site.id); db.prepare("UPDATE site_pages SET last_checked_at=CURRENT_TIMESTAMP WHERE site_id=? AND crawl_status='indexed'").run(site.id); checked += 1; added += siteAdded; details.push({ site: root, sitemaps: result.maps, new_pages: siteAdded }); } catch (error) { db.prepare('UPDATE sites SET last_checked_at=CURRENT_TIMESTAMP WHERE id=?').run(site.id); details.push({ site: root, error: String(error).slice(0, 300) }); } }
