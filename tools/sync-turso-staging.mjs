...
const encode=value=>value===null?{type:'null'}:typeof value==='number'?(Number.isInteger(value)?{type:'integer',value:String(value)}:{type:'float',value:String(value)}):{type:'text',value:String(value)};
...
let total=0;for(const [,database,tables] of groups)for(const table of tables){const statements=tableStatements(database,table);await send(statements);total+=statements.length;console.log(`${table}: ${statements.length}`);}
